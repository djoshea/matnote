...
%=========================================================================
function result = WriteYaml(filename, data, flowstyle, varargin)
    p = inputParser;
    ...
    % use "flowstyle" to serialize
    p.addOptional('flowstyle', 0, @(x) ismember(x, [0 1]));

    % append to file if existing
    p.addParamValue('append', false, @islogical);
    p.parse(filename, data, varargin{:});

    flowstyle = p.Results.flowstyle;
